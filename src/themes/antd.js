// Antd framework theme
import { AbstractTheme } from '../theme.js'
import rules from './antd.css.js'
...
export class AntdTheme extends AbstractTheme {
  ...
  setGridColumnSize (el, size, offset) {
    el.classList.add(`ant-col-${size * 2}`)
  ...
  getGridRow () {
    const el = document.createElement('div')
    el.classList.add('ant-row')
    return el
  }

  getGridColumn () {
    const el = document.createElement('div')
    el.classList.add('ant-col')
    if (this.options.align_bottom) el.classList.add('ant-row-bottom')
    return el
  }
  ...
  getIndentedPanel () {
    ...
    if (this.options.object_border) el.classList.add('ant-card-bordered')
    return el
  }
  ...
  getFormButton (text, icon, title) {
    ...
    el.classList.add('ant-ant-btn', 'mx-2', 'my-1')
    if (this.options.input_size !== 'small') el.classList.remove('ant-ant-btn-sm')
    if (this.options.input_size === 'large') el.classList.add('ant-ant-btn-lg')
    return el
  }

  getButton (text, icon, title) {
    const el = super.getButton(text, icon, title)
    el.classList.add('ant-btn', 'ant-btn-sm', 'mr-2', 'my-1')
    return el
  }
  ...
  getFormInputLabel (text, req) {
    ...
    if (this.options.label_bold) el.classList.add('ant-form-item-label')
    return el
    // return null
  }
  ...
  getCheckbox () {
    ...
    // el.classList.atdd('ant-checkbox-input')
    return el
  }
  ...
  getInputGroup (input, buttons) {
    ...
    for (let i = 0; i < buttons.length; i++) {
      buttons[i].classList.add('ant-input-group-btn')
      buttons[i].classList.remove('ant-btn-sm', 'mr-2', 'my-1')
  ...
  getInfoButton (text) {
    ...
    button.classList.add('ant-btn', 'ant-btn-secondary', 'ant-btn-info', 'ant-btn-action', 's-circle')
  ...
  getTabHolder (propertyName) {
    ...
    const el = document.createElement('div')
    el.classList.add('ant-row')
    el.innerHTML = `<div class="ant-col ant-col-2"></div><div class="ant-col col-10 content" id="${pName}"></div>`
    return el
  }
  ...
  getTab (span, tabId) {
    ...
    el.classList.add('ant-btn', 'ant-btn-secondary', 'ant-btn-block')
